...
import { calculateVolatilityRatio, getDynamicParameters, calculateThrottledRisk } from './math-engine.js';
import { runDeliberation } from './ai-router.js';
import { exposePerformanceTestToWindow } from './performanceTestRunner.js';
import { exposeSecurityAPIToWindow } from './securityMonitor.js';
import { initLeakagePrevention } from './leakagePreventionModule.js';
import { initSocialEngineeringDetection } from './socialEngineeringDetectionModule.js';
import { SecuritySentinel, AntivirusGateway, AntiSpamShield } from './securitySentinel.js';
import { testTelegramConnectivity, enableContinuousMonitoring } from './telegramDiagnostics.js';
import { initTelegramMonitor } from './telegramMonitor.js';
import ThemeSwitcher from './ThemeSwitcher.jsx';
import { formatPhoneNumber, TradersRegimentWatermark, ExchangeFacilityBadge } from './businessLogicUtils.jsx';
...
// ═══════════════════════════════════════════════════════════════════
// ═══════════════════════════════════════════════════════════════════
// FORENSIC DATA COLLECTION FOR BREACH ALERTS
// ═══════════════════════════════════════════════════════════════════
...
// Expose to window for SecuritySentinel to access
...
const sendForensicAlert = async (targetEmail, alertType = 'BREACH') => {
  ...
  // Build world-class formatted message
  ...
  const toolsCategories = [
    {
      ...
      items: [
        ...
        { label: 'Performance', icon: '⚡', action: () => console.log('Performance') },
        { label: 'Reports', icon: '📋', action: () => console.log('Reports') }
      ]
    },
    {
      name: 'Management',
      icon: '⚙️',
      items: [
        { label: 'Users', icon: '👥', action: () => console.log('Users') },
        { label: 'Permissions', icon: '🔐', action: () => console.log('Permissions') },
    ...
    },
    {
      ...
      items: [
        ...
        { label: 'Exports', icon: '📤', action: () => console.log('Exports') },
        { label: 'Imports', icon: '📥', action: () => console.log('Imports') }
      ]
    },
    {
      name: 'Security',
      icon: '🔒',
      items: [
        { label: 'Audit Log', icon: '📝', action: () => console.log('Audit Log') },
        { label: 'Encryption', icon: '🔐', action: () => console.log('Encryption') },
        { label: 'Access Control', icon: '🛡️', action: () => console.log('Access Control') }
      ]
    }
  ];
...
const createTheme = (isDark = true, accentKey = 'BLUE') => {
  ...
  // Text colors
  muted: isDark ? "#8E8E93" : "#555555",
  dim: isDark ? "#3A3A3C" : "#ADADAD",
  text: isDark ? "#F2F2F7" : "#000000",
  textSecondary: isDark ? "#A1A1A6" : "#555555",
...
// Default theme - will be overridden in component with system dark mode
const T = createTheme(true, 'BLUE');
...
// ═══════════════════════════════════════════════════════════════════
const authCard = { background: "rgba(255, 255, 255, 0.9)", border: `1px solid #E5E7EB`, borderRadius: 16, padding: "clamp(48px, 10vw, 80px)", width: "100%", maxWidth: 460, margin: "0 auto", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", boxShadow: "0 20px 25px -5px rgba(0, 0, 0, 0.05)" };
const authInp = { background: "rgba(255,255,255,0.08)", border: `1px solid rgba(255,255,255,0.12)`, borderRadius: 10, padding: "14px 16px", color: T.text, fontSize: 15, width: "100%", boxSizing: "border-box", outline: "none", transition: "all 0.2s ease", marginBottom: 16, backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)" };
const authBtn = (color, disabled) => ({ background: disabled ? "rgba(0,0,0,0.3)" : `${color}1A`, border: `1px solid ${disabled ? "rgba(255,255,255,0.1)" : `${color}60`}`, borderRadius: 10, padding: "16px 0", cursor: disabled ? "not-allowed" : "pointer", color: disabled ? T.muted : color, fontFamily: T.font, fontSize: 14, fontWeight: 700, letterSpacing: 2, width: "100%", transition: "all 0.2s ease", opacity: disabled ? 0.6 : 1, backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", className: "btn-glass" });
const lbl = { color: T.muted, fontSize: 11, letterSpacing: 1.5, marginBottom: 6, display: "block", textTransform: "uppercase", fontWeight: 600, fontFamily: T.font };
const inp = { background: "rgba(255,255,255,0.07)", border: `1px solid rgba(255,255,255,0.12)`, borderRadius: 8, padding: "12px 14px", color: T.text, fontFamily: T.mono, fontSize: 14, width: "100%", boxSizing: "border-box", outline: "none", transition: "all 0.2s ease", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)" };
const cardS = (e = {}) => ({ background: "rgba(255,255,255,0.05)", border: `1px solid rgba(255,255,255,0.1)`, borderRadius: 12, padding: "20px 24px", marginBottom: 16, backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", boxShadow: "0 4px 16px rgba(0,0,0,0.1)", ...e });
const glowBtn = (color, disabled) => ({ background: disabled ? "rgba(0,0,0,0.2)" : `${color}12`, border: `1px solid ${disabled ? "rgba(255,255,255,0.08)" : `${color}40`}`, borderRadius: 8, padding: "14px 28px", cursor: disabled ? "not-allowed" : "pointer", color: disabled ? T.muted : color, fontFamily: T.font, fontSize: 13, fontWeight: 600, letterSpacing: 1.5, transition: "all 0.2s ease", opacity: disabled ? 0.6 : 1, backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", className: "btn-glass" });
...
const AuthLogo = () => (
  <div style={{ textAlign: "center", marginBottom: 36 }}>
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 12, marginBottom: 12 }}>
      ...
      />
      {/* Logo Text using Inter font */}
      <div style={{ color: "#111827", fontSize: "clamp(18px, 4vw, 22px)", letterSpacing: 2, fontFamily: "Inter, -apple-system, BlinkMacSystemFont, sans-serif", fontWeight: 700 }}>
        TRADERS REGIMENT
      </div>
    </div>
    <div style={{ color: "var(--muted)", fontSize: 11, letterSpacing: 4, marginTop: 6, fontWeight: 600 }}>INSTITUTIONAL FUTURES TERMINAL · v9</div>
  </div>
);
    ...
    <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: T.font }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 5, alignItems: "flex-end", height: 30, marginTop: 24 }}>
          {[10, 18, 12, 24, 15, 20, 11].map((h, i) => <div key={i} style={{ width: 5, height: h, background: T.green, borderRadius: 3, animation: `bar 0.85s ${i * 0.1}s ease-in-out infinite alternate` }} />)}
        </div>
        <div style={{ color: T.muted, fontSize: 11, letterSpacing: 4, marginTop: 16, fontWeight: 600 }}>INITIALIZING...</div>
      </div>
    </div>
  ); 
}
...
// ═══════════════════════════════════════════════════════════════════
function LoginScreen({ onLogin, onSignup, onAdmin }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: T.font, padding: 20 }}>
      <div style={authCard} className="glass-panel">
        <AuthLogo />
        <div style={{ color: T.muted, fontSize: 11, letterSpacing: 2, textAlign: "center", marginBottom: 28, fontWeight: 600 }}>
          {resetMode ? "ACCOUNT RECOVERY" : "ENTERPRISE GATEWAY · INSTITUTIONAL-GRADE MFA PROTOCOL"}
        </div>
        ...
        <div>
          ...
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="your@email.com" autoComplete="email" style={{...authInp, fontFamily: T.font}} className="input-glass" />
        </div>
        ...
        {!resetMode ? (
          <>
            <div>
              ...
              <div style={{ position: 'relative', width: '100%', marginBottom: 15 }}>
                <input 
                  ...
                  style={{ ...authInp, width: '100%', marginBottom: 0, fontFamily: T.mono, letterSpacing: 4 }} 
                  placeholder="••••••••••••"
            ...
            <button onClick={submit} disabled={loading} style={authBtn(T.green, loading)} className="btn-glass">{loading ? "⟳ AUTHENTICATING..." : "→ ACCESS TERMINAL"}</button>
            
            {/* RULE #23: Google Sign-In */}
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: T.font, padding: 20 }}>
      <div style={{ ...authCard, maxWidth: 540, textAlign: "center" }} className="glass-panel">
        <AuthLogo />
        <div style={{ width: 80, height: 80, borderRadius: "50%", border: `3px solid rgba(255,214,10,0.4)`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 24px", background: "rgba(255,214,10,0.1)" }}>
          <span style={{ color: T.gold, fontSize: 32 }}>⏳</span>
        </div>
        <div style={{ color: T.gold, fontSize: 16, letterSpacing: 3, marginBottom: 20, fontWeight: 700 }}>
          APPLICATION PENDING
        </div>
        <div style={{ color: "#A1A1A6", fontSize: 14, lineHeight: 1.8, marginBottom: 28, padding: "20px 24px", background: "rgba(0,0,0,0.4)", border: `1px solid rgba(255,255,255,0.08)`, borderRadius: 12 }} className="glass-panel">
          It will take time to check if you are eligible to use the app or not.
        </div>
        <div style={{ color: T.muted, fontSize: 12, lineHeight: 1.9, marginBottom: 32 }}>
        ...
        <div style={{ display: "flex", gap: 12, flexDirection: "column" }}>
          <button onClick={async () => { setChecking(true); await onRefresh(); setChecking(false); }} disabled={checking} style={authBtn(T.blue, checking)} className="btn-glass">
            {checking ? "⟳ CHECKING STATUS..." : "↺ CHECK APPROVAL STATUS"}
          </button>
          <button onClick={onLogout} style={{ ...authBtn(T.muted, false), background: "transparent" }} className="btn-glass">
            ← LOGOUT
          </button>
        </div>
      </div>
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          borderRadius: "50%",
          border: "3px solid rgba(0,122,255,0.2)",
          borderTopColor: "rgba(0,122,255,0.8)",
          animation: "spin 1s linear infinite"
        }} />
      </div>
...
function SupportChatModal({ isOpen, userId, userName, onClose, auth, showToast }) {
  ...
  useEffect(() => {
    ...
    const chatRef = ref(firebaseDb, chatPath);
    ...
    const typingRef = ref(firebaseDb, `${chatPath}/typing_${userId}`);
...
function AdminDashboard({ auth, onLogout, isAdminAuthenticated, showToast, maintenanceModeActive, handleToggleMaintenanceMode }) {
      ...
      const unsubscribe = onValue(usersRef, 
        ...
        (error) => {
          ...
          if (error.code === 'PERMISSION_DENIED') {
            setDbError('🔐 Database Access Denied - Check Firebase Rules');
          } else {
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: T.font, animation: "fadeInDashboard 0.6s ease-out" }}>
      ...
      <div style={{ background: "rgba(0,0,0,0.6)", borderBottom: `1px solid rgba(191,90,242,0.3)`, padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }} className="glass-panel">
                        ...
                        <thead>
                          <tr>
                            {["Date", "Inst", "Dir", "Type", "AMD", "RRR", "Entry", "Exit", "P&L", "Result"].map(h => (
                              <th key={h} style={{ padding: "12px 14px", textAlign: "left", color: T.dim, fontSize: 10, letterSpacing: 1, background: "rgba(0,0,0,0.4)", borderBottom: `1px solid rgba(255,255,255,0.05)`, whiteSpace: "nowrap", fontWeight: 700 }} className="gemini-gradient-text">{h}</th>
                            ))}
                          </tr>
                        </thead>
                        <tbody>
                          {journal.slice(-20).reverse().map((t, i) => { 
                            ...
                            return (
                              <tr key={i} style={{ borderBottom: `1px solid rgba(255,255,255,0.05)`, background: i % 2 === 0 ? "rgba(255,255,255,0.02)" : "transparent" }}>
                                <td style={{ padding: "10px 14px", color: T.muted, fontSize: 11, fontFamily: T.mono }}>{t.date}</td>
                                <td style={{ padding: "10px 14px", color: T.text, fontSize: 11, fontWeight: 700 }}>{t.instrument}</td>
                                <td style={{ padding: "10px 14px", color: t.direction === 'Long' ? T.green : T.red, fontSize: 11, fontWeight: 600 }}>{t.direction}</td>
                                <td style={{ padding: "10px 14px", color: T.blue, fontSize: 11, fontWeight: 500 }}>{t.tradeType}</td>
                                ...
                                <td style={{ padding: "10px 14px", color: T.gold, fontSize: 11, fontFamily: T.mono }}>{t.rrr}</td>
                                <td style={{ padding: "10px 14px", color: T.muted, fontSize: 11, fontFamily: T.mono }}>{t.entry || "—"}</td>
                                <td style={{ padding: "10px 14px", color: T.muted, fontSize: 11, fontFamily: T.mono }}>{t.exit || "—"}</td>
                                <td style={{ padding: "10px 14px", color: pv >= 0 ? T.green : T.red, fontSize: 12, fontWeight: 700, fontFamily: T.mono }}>{pv >= 0 ? "+" : ""}${pv.toFixed(0)}</td>
                                <td style={{ padding: "10px 14px", color: t.result === 'win' ? T.green : t.result === 'loss' ? T.red : T.muted, fontSize: 11, fontWeight: 800 }}>{(t.result || "—").toUpperCase()}</td>
                              </tr>
                            ); 
                          })}
                        </tbody>
...
function MainTerminal({ profile, onLogout, onSaveJournal, onSaveAccount, onSaveFirmRules, showToast }) {
  ...
  // State variables: setters unused, getters needed for render
  const [currentAMD, _setCurrentAMD] = useState('UNCLEAR');
  const [parsed, _setParsed] = useState(null);
  const [p1Out, _setP1Out] = useState('');
  const [_p1NewsChart, setP1NewsChart] = useState(null); 
  const [_p1PremarketChart, setP1PremarketChart] = useState(null); 
  const [_p1KeyLevelsChart, setP1KeyLevelsChart] = useState(null);
  ...
  const extractFromScreenshots = async () => {
    ...
    try {
      ...
      // Indicator extraction failed silently
    } finally { 
      setExtracting(false); 
    }
  };
...
Consistency: ${todayPct !== null ? `${todayPct.toFixed(1)}% of target today (cap: ${consPct}%)` : 'N/A'}`;
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.bg, color: T.text, fontFamily: T.font }}>
      ...
      {/* Terminal Header */}
      <div style={{ background: "rgba(0,0,0,0.6)", borderBottom: `1px solid rgba(255,255,255,0.08)`, padding: "16px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }} className="glass-panel">
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <div>
            <div style={{ color: T.green, fontSize: 16, letterSpacing: 4, fontWeight: 800 }}>{fullGreeting}</div>
            <div style={{ color: T.muted, fontSize: 10, letterSpacing: 2, marginTop: 4, fontWeight: 600 }}>INSTITUTIONAL TERMINAL · v9</div>
          </div>
      ...
      {/* Navigation Tabs */}
      <div style={{ background: "rgba(0,0,0,0.4)", borderBottom: `1px solid rgba(255,255,255,0.08)`, padding: "0 32px", display: "flex", overflowX: "auto" }} className="glass-panel">
        {[
          ...
          <button 
            ...
            className="btn-glass"
          >
            <div style={{ color: part === p.id ? p.color : T.muted, fontSize: 12, letterSpacing: 1.5, fontWeight: 800 }}>
              {p.label}
            </div>
            <div style={{ color: part === p.id ? "rgba(255,255,255,0.7)" : T.dim, fontSize: 10, marginTop: 4, fontWeight: 500 }}>
              {p.sub}
            </div>
          </button>
        ))}
      ...
      {/* ========================================================================= */}
        ...
        {part === '2' && (
          <div>
            ...
            {fr.parsed && (
              <div style={{ display: "flex", gap: 16, padding: "14px 20px", background: "rgba(0,0,0,0.4)", border: `1px solid ${complianceColor}40`, borderRadius: 10, marginBottom: 16, flexWrap: "wrap" }} className="glass-panel">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, margin: "16px 0", padding: "12px 20px", background: "rgba(0,0,0,0.4)", border: `1px solid rgba(255,255,255,0.1)`, borderRadius: 8 }} className="glass-panel">
            ...
            <div ref={p2Ref} style={{ marginTop: 24 }}>
              {loading && <Loader color={T.orange} label="RUNNING AMD ANALYSIS..." />}
              {!loading && p2Out && (
                <div>
                  ...
                  {showP2TradeForm && (
                    <div style={{ background: "rgba(0,0,0,0.6)", border: `1px solid rgba(191,90,242,0.4)`, borderRadius: 12, padding: "20px 24px", marginBottom: 20 }} className="glass-panel">
        ...
        {part === '3' && (() => {
          ...
          return (
            <div>
              ...
              {journal.length === 0 ? (
                <div style={{ background: "rgba(0,0,0,0.4)", border: `1px solid rgba(255,255,255,0.1)`, borderRadius: 12, padding: "60px", textAlign: "center", color: T.dim, fontSize: 14, fontWeight: 600 }} className="glass-panel">
                  No trades logged yet
                </div>
              ) : (
                <div style={{ background: "rgba(0,0,0,0.4)", border: `1px solid rgba(255,255,255,0.1)`, borderRadius: 12, overflow: "hidden" }} className="glass-panel">
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse" }}>
                      <thead>
                        <tr style={{ borderBottom: `1px solid rgba(255,255,255,0.1)` }}>
                          {["DATE", "INST", "DIR", "TYPE", "AMD", "ENTRY", "EXIT", "P&L", "RESULT", ""].map((h, i) => (
                            <th key={i} style={{ padding: "14px 16px", textAlign: "left", color: T.dim, fontSize: 10, letterSpacing: 1.5, fontFamily: T.font, fontWeight: 700, whiteSpace: "nowrap", background: "rgba(0,0,0,0.6)" }} className="gemini-gradient-text">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {[...journal].reverse().map((t, i) => { 
                          ...
                          return (
                            <tr key={i} style={{ borderBottom: `1px solid rgba(255,255,255,0.05)`, background: i % 2 === 0 ? "rgba(255,255,255,0.02)" : "transparent" }}>
                              <td style={{ padding: "12px 16px", color: T.muted, fontSize: 11, whiteSpace: "nowrap", fontFamily: T.mono }}>{t.date}</td>
                              <td style={{ padding: "12px 16px", color: T.text, fontSize: 12, fontWeight: 700 }}>{t.instrument}</td>
                              ...
                              <td style={{ padding: "12px 16px" }}>
                                <span style={{ color: isW ? T.green : isL ? T.red : T.muted, fontSize: 11, fontWeight: 800 }}>
                                  {isW ? "WIN" : isL ? "LOSS" : "BE"}
                                </span>
                              </td>
                              <td style={{ padding: "12px 16px" }}>
...
export default function TradersRegiment() {
  ...
  // CREATE DYNAMIC THEME BASED ON SYSTEM DARK MODE & ACCENT COLOR
  const THEME = useMemo(() => createTheme(systemIsDark, accentColor), [systemIsDark, accentColor]);
  ...
  useEffect(() => {
    ...
    const initializeHeartbeat = async () => {
      try {
        // Get the database reference for the .info/connected path
        // This path is automatically maintained by Firebase and is true when connected
        const connPath = ref(firebaseDb, '.info/connected');
        
        // Listen to Firebase connection status
        unsubscribe = onValue(connPath, (snapshot) => {
          const isConnected = snapshot.val() === true;
          lastHeartbeatTime = Date.now();
          
          if (isConnected) {
            // Firebase is actively connected
            window.dispatchEvent(new CustomEvent('firebaseConnected', {
              detail: { timestamp: lastHeartbeatTime, status: 'healthy' }
            }));
          } else {
            // Firebase connection lost - may be temporary
  ...
  useEffect(() => {
    const unsubscribe = firebaseAuth.onAuthStateChanged(async (user) => {
      if (user) {
        const token = await user.getIdToken();
        const authData = { uid: user.uid, token: token, email: user.email };
        setAuth(authData);
        await checkUserStatus(authData);
      } else {
        setScreen('login');
  ...
  const handleSignup = async (formData) => {
    ...
    const { fullName, stayLoggedIn } = formData;
  ...
  const handleOTPVerified = async () => {
    ...
    if (userRole === 'admin') {
      ...
      // Default to app for unknown roles
      setScreen('app');
    }
  };
  ...
  // ─── MAIN ROUTER RENDER ───
  // Show splash screen while checking for persisted admin session
  if (isInitialLoading) {
    return <SplashScreen />;
  }
  ...
  const screenContent = (() => {
    ...
    case 'signup': 
      return <SignupScreen onBack={() => setScreen('login')} onSubmit={handleSignup} />;
    ...
    case 'sessions':
      return <SessionsManagementScreen profile={profile} auth={auth} currentSessionId={currentSessionId} onBack={() => setScreen('app')} showToast={showToast} />;
  ...
  return (
    <div className={`app-container theme-${theme}`}>
      ...
      {/* Traders Regiment Brand Watermark & Founder Signature */}
      <div style={{
        marginTop: 'auto',
