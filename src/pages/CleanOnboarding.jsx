...
const CleanOnboarding = ({
  ...
  const isFormValid =
    email && password && password.length >= 8 && agreedToTerms;
  ...
  // Theme variables (matching AURA Engine design system)
  const T = {
    bg: "var(--aura-surface-elevated, #FFFFFF)",
    fg: "var(--aura-text-primary, #111827)",
    muted: "var(--aura-text-secondary, #64748B)",
    blue: "var(--aura-accent-primary, #2563EB)", // Emergency Blue for links
    green: "var(--aura-accent-success, #22C55E)",
    red: "var(--aura-accent-danger, #EF4444)",
    border: "var(--aura-border-subtle, #E2E8F0)",
    cardBg: "var(--aura-surface-primary, #F8FAFC)",
  };
  ...
  const authCard = {
    ...
    borderRadius: 12,
    padding: "40px 32px",
    boxShadow: "var(--aura-shadow-card, 0 4px 16px rgba(0, 0, 0, 0.08))",
  };
  ...
  const authBtn = (bgColor, disabled = false) => ({
    ...
    color: disabled
      ? "var(--aura-text-disabled, #94A3B8)"
      : "var(--aura-text-on-accent, #FFFFFF)",
    fontSize: 14,
    fontWeight: 600,
    letterSpacing: 0.5,
    border: "none",
    ...
    >
      <div style={{ maxWidth: 440, width: "100%" }}>
        ...
        <div style={authCard}>
          ...
          >
            We'll request camera, microphone, or location permissions only when
