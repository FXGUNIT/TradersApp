...
export default function DebugOverlay({
  ...
  const logsByType = {
    log: logs.filter((entry) => entry.type === "log"),
    warn: logs.filter((entry) => entry.type === "warn"),
    error: logs.filter((entry) => entry.type === "error"),
    info: logs.filter((entry) => entry.type === "info"),
  };

  const avgLatency =
    latencies.length > 0
      ? (latencies.reduce((sum, entry) => sum + entry.ms, 0) / latencies.length).toFixed(0)
      : 0;

  const slowRequests = latencies.filter((entry) => entry.ms > 2000);
  const errorCount =
    Object.keys(logsByType)
      .map((key) => logsByType[key].length)
      .reduce((sum, count) => sum + count, 0) === logs.length
      ? logs.filter((entry) => entry.type === "error").length
      : 0;
        ...
        >
          ...
          >
            <div>
              ...
              <div style={{ color: "#BF5AF2", fontSize: 14, fontWeight: 700 }}>{tti}ms</div>
            </div>
            <div>
              <div style={{ color: "#A1A1A6", fontSize: 10, marginBottom: 4 }}>
                Avg API Latency
            ...
            <div>
              ...
              <div
                style={{
                  color: errorCount > 0 ? "#FF453A" : "#30D158",
                  fontSize: 14,
                  fontWeight: 700,
                }}
              >
            ...
            {expandedSection === "Network" && (
              <div style={{ padding: "8px" }}>
                ...
                {slowRequests.length === 0 ? (
                  ...
                  slowRequests.slice(-8).map((request, index) => (
                    <div
                      key={index}
                      style={{
                        padding: "6px",
                    ...
                    >
                      {"\uD83D\uDC22"} {request.endpoint || "API"}: <strong>{request.ms}ms</strong>
                    </div>
                  ))
                )}
                <div
                ...
                {latencies.slice(-8).map((request, index) => (
                  <div
                    key={index}
                    style={{
                      padding: "4px 6px",
                      background:
                        request.ms > 2000 ? "rgba(255,69,58,0.05)" : "rgba(48,209,88,0.05)",
                      color: request.ms > 2000 ? "#FF453A" : "#30D158",
                      fontSize: 9,
                      borderLeft: `2px solid ${request.ms > 2000 ? "#FF453A" : "#30D158"}`,
                      paddingLeft: 8,
                    }}
                  >
                    {request.endpoint || "request"}: {request.ms}ms
                  </div>
                ))}
              </div>
            )}
                ...
                {Object.entries(componentStatus).length === 0 ? (
                  ...
                  Object.entries(componentStatus)
                    ...
                    .map(([key, component], index) => (
                      ...
                      >
                        <div style={{ fontWeight: 600, color: "#BF5AF2" }}>{component.name}</div>
                        <div
                          style={{
                            fontSize: 8,
                            color: "#A1A1A6",
