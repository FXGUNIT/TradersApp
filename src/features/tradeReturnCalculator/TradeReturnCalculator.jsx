...
export const TradeReturnCalculator = React.memo(function TradeReturnCalculator({
  consensus,
}) {
  ...
  return (
    <SectionCard
      title="Trade Return Calculator"
      icon={Calculator}
      accent="rgba(48,209,88,0.7)"
    >
      {/* Collapse toggle */}
      <button
        onClick={() => {
          setOpen((v) => !v);
          if (!open && !result) runSim();
        }}
        style={{
          display: "flex",
          alignItems: "center",
          ...
          >
            <span>
              ...
              <strong style={{ color: "#30D158" }}>
                {k > 0 ? `${(k * 100).toFixed(1)}%` : "N/A"}
              </strong>
            </span>
            <span>
              Risk/trade:{" "}
              <strong>{formatCurrency(inputs.balance * inputs.riskPct)}</strong>
            </span>
            <span>
              Win:{" "}
              <strong style={{ color: "#30D158" }}>
                {formatCurrency(pnl.win)}
              </strong>
            </span>
            <span>
              Loss:{" "}
                ...
                {[
                  {
                    ...
                    color:
                      result.endBalance >= inputs.balance
                        ? "#30D158"
                        : "#FF453A",
                  },
                  {
                    ...
                    color: result.roi >= 0 ? "#30D158" : "#FF453A",
                  },
                  {
                    label: "Max Drawdown $",
                  ...
                  {
                    label: "Wins",
                    value: formatNum(result.nWins),
                    color: "#30D158",
                    icon: TrendingUp,
                  },
                  {
                    label: "Losses",
                    value: formatNum(result.nLosses),
                    color: "#FF453A",
                    icon: TrendingDown,
                  },
