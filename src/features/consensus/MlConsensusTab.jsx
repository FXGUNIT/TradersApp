...
import { fetchCalendarSnapshot } from "../../services/calendarGateway.js";
import { EventCalendarCompact } from "../calendar/EventCalendarCompact.jsx";
import { ExpiryCalendarPanel } from "../calendar/ExpiryCalendarPanel.jsx";
import { InstrumentQuickStats } from "../dashboard/InstrumentQuickStats.jsx";
import { InstrumentSwitcher } from "../dashboard/InstrumentSwitcher.jsx";
import { MarketTimelineClock } from "../dashboard/MarketTimelineClock.jsx";
import { SessionStatusPanel } from "../dashboard/SessionStatusPanel.jsx";
import { useActiveInstrument } from "../dashboard/ActiveInstrumentContext.jsx";
...
export const MlConsensusTab = React.memo(function MlConsensusTab({ theme: _theme, normalizedTheme: _normalizedTheme }) {
  const { activeInstrument } = useActiveInstrument();
  const [consensus, setConsensus] = useState(null);
  const [calendarNow, setCalendarNow] = useState(null);
  const [expiryCalendar, setExpiryCalendar] = useState([]);
  const [holidayCalendar, setHolidayCalendar] = useState([]);
  const [calendarObservedAtMs, setCalendarObservedAtMs] = useState(Date.now());
  ...
  const doFetch = useCallback(async () => {
    ...
    try {
      const [consensusData, calendarData] = await Promise.all([
        fetchConsensus({ session: 1, symbol: activeInstrument.symbol }),
        fetchCalendarSnapshot({ count: 4 }),
      ]);

      setConsensus(consensusData);
      setCalendarNow(calendarData?.now || null);
      setExpiryCalendar(calendarData?.expiry?.expiries || []);
      setHolidayCalendar(calendarData?.holidays?.holidays || []);
      setCalendarObservedAtMs(Date.now());
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ fontSize: 10, color: "var(--text-tertiary)" }}>
            {activeInstrument.shortLabel} · Updated {sinceRefresh}
          </span>
          <button
            onClick={doFetch}
            disabled={loading}
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 24px" }}>
        ...
        <div className="cc-dashboard-grid">
          <MarketTimelineClock marketNow={calendarNow} observedAtMs={calendarObservedAtMs} />
          <SessionStatusPanel marketNow={calendarNow} activeInstrument={activeInstrument} />
          <InstrumentQuickStats activeInstrument={activeInstrument} consensus={consensus} />
        </div>

        <div className="cc-dashboard-grid cc-dashboard-grid--secondary">
          <EventCalendarCompact holidays={holidayCalendar} marketNow={calendarNow} />
          <ExpiryCalendarPanel expiries={expiryCalendar} />
        </div>
