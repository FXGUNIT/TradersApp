/**
 * TradersApp BFF proxy — Cloudflare Workers (ES module, no compatibility flags).
 *
 * Deploy: curl PUT /workers/scripts/tradersapp-bff-proxy/content
 * Auth:   Authorization: Bearer <cfat_ token>
 */

const PROXY_PATH_PREFIXES = [
  "/admin", "/ai-status", "/board-room", "/calendar",
  "/content", "/health", "/identity", "/ml", "/news",
  "/onboarding", "/support", "/telegram", "/terminal",
  "/terminal-analytics", "/trade-calc",
];
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      return new Response(JSON.stringify({
        ok: false,
        error: isTimeout
          ? "BFF service temporarily unreachable via proxy."
          : "Proxy error.",
        path: url.pathname,
      }), {
        status: 502,
