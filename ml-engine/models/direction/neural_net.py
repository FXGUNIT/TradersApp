"""
MLP Neural Net Direction Classifier.
...
class MLPClassifierModel:
    """
    MLP-based directional classifier.
    Architecture: 2 hidden layers (64, 32) with ReLU + BatchNorm-like scaling.
    ...
    def train(
        ...
        # Feature importance via activation-based sensitivity (approximate)
        ...
        if verbose:
            print(f"  MLP CV ROC-AUC: {result['cv_roc_auc_mean']:.4f} ± {result['cv_roc_auc_std']:.4f}")

        return result

    def _approximate_importance(self, X: pd.DataFrame) -> np.ndarray:
        """Approximate MLP feature importance using weight magnitude proxy."""
        clf = self.pipeline.named_steps["clf"]
        if hasattr(clf, "estimators_") and clf.estimators_:
            estimator = clf.estimators_[0]
            if hasattr(estimator, "coefs_"):
                # Average absolute weight magnitude per input feature
                weights = np.abs(estimator.coefs_[0])
