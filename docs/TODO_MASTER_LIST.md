...
Last sync: `2026-04-09 21:39:22 India Standard Time`
...
| P03 | Self-hosted CI/CD with Gitea + Woodpecker | Partial | `63-70` plus `H01-H04` below | Live runner setup, registry wiring, smoke/rollback/canary completion | 3-5d |
...
| P12 | Multi-level automated testing | Partial | `54-62`, `66-68`, `H10` below | CI gates, critical-path expansion, environment-backed smoke/perf rules | 4-6d |
| P13 | Security by design with Keycloak + Trivy | Partial | `69-70` plus `H05-H07` below and existing security docs | Enforce live scanning/policies, cluster auth path, stronger service-to-service security | 3-5d |
...
| P15 | Docs-as-code with MkDocs | Mostly done | existing docs tree, `mkdocs.yml`, plus `H08` below | Publish/build path, fill gaps, keep docs tied to releases | 1-2d |
...
| P20 | Reviews with ADR enforcement | Partial | existing process plus `H11-H12` below | Add explicit review checklist and enforce ADR updates on major merges | 1-2d |
...
- Run `python scripts/update_todo_progress.py` to refresh the dashboard at the top of this file.
