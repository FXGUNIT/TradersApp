...
Robust ensemble — handles non-linearity, feature importance built-in.
"""
import numpy as np
import pandas as pd
import sys, os
...
class RandomForestClassifierModel:
    ...
    Calibrated via Platt scaling (sigmoid) for probability estimates.
    """
    ...
    def __init__(self):
        base = RandomForestClassifier(**config.RF_DIRECTION)
        self.pipeline = CalibratedClassifierCV(base, method="sigmoid", cv=5)
    ...
    def train(
        ...
        if verbose:
            print(f"  RandomForest CV ROC-AUC: {result['cv_roc_auc_mean']:.4f} ± {result['cv_roc_auc_std']:.4f}")
