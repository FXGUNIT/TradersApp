...
Performance optimizations (20 experiments, exp-1 through exp-20):
  exp-4-8:   Stage-level LRU caches on 5 pipeline stages
  exp-9:     Full-call memoization for engineer_features (identical inputs)
  exp-10-20: Fast-hint path — cheap hint comparison before fingerprint construction
  exp-3:     Historical rolling stats cached by trade-log fingerprint
  exp-2:     merge_asof replaced with searchsorted for candle/trade alignment
  exp-1:     Vectorized profit_factor rolling (avoid .apply with raw=False)
"""
import numpy as np
import pandas as pd
...
# -------------------------------------------------------------------------
# Global cache state (shared across calls, process-lifetime)
# -------------------------------------------------------------------------

_STAGE_CACHE: dict[int, list] = {}   # stage_id → list of (fingerprint, result)
_HIST_STATS_CACHE: dict[str, pd.DataFrame] = {}  # trade-log fingerprint → precomputed stats
_FULL_CACHE: dict[str, pd.DataFrame] = {}  # full pipeline fingerprint → result
...
def _fingerprint_df(df: pd.DataFrame) -> str:
    """Stable fingerprint for a DataFrame (len + first/last values + dtypes)."""
    if df.empty:
        return "empty"
    n = len(df)
    ts_col = "timestamp"
    if ts_col in df.columns:
        ts_vals = df[ts_col].iloc[[0, -1]].astype(str).tolist()
    else:
        ts_vals = []
    shape_hash = hashlib.md5(str((n, tuple(ts_vals), tuple(df.columns.tolist()))).encode()).hexdigest()[:12]
    return f"n{n}_{shape_hash}"


def _fingerprint_dict(d: dict | None) -> str:
    """Fingerprint a dict by its length and key/value summary."""
    if not d:
        return "none"
    keys = sorted(d.keys())[:6]
    parts = [f"{k}={str(d[k])[:20]}" for k in keys]
    return hashlib.md5("|".join(parts).encode()).hexdigest()[:16]


def _fast_hint_candles(candles_df: pd.DataFrame) -> str:
    """exp-12: Drop volume from fast candle hint + shallow hit copy."""
    if candles_df.empty:
        return "empty"
    # Minimal fingerprint: len + first/last close + first/last timestamp
    n = len(candles_df)
    close0 = str(candles_df["close"].iloc[0])[:8] if "close" in candles_df.columns else "?"
    close_last = str(candles_df["close"].iloc[-1])[:8] if len(candles_df) > 1 else close0
    ts0 = str(candles_df["timestamp"].iloc[0])[:16] if "timestamp" in candles_df.columns else "?"
    return f"c{n}_{close0}_{close_last}_{ts0}"


def _fast_hint_trades(trade_log_df: pd.DataFrame | None) -> str:
    """exp-18: Trade hint len+ts."""
    if trade_log_df is None or trade_log_df.empty:
        return "no_trades"
    n = len(trade_log_df)
    ts_col = "entry_time" if "entry_time" in trade_log_df.columns else "timestamp"
    if ts_col in trade_log_df.columns:
        ts_last = str(trade_log_df[ts_col].iloc[-1])[:16]
    else:
        ts_last = "?"
    return f"t{n}_{ts_last}"


def _fast_hint_session(session_agg_df: pd.DataFrame | None) -> str:
    """exp-15: Fast return deep + session hint len+ts."""
    if session_agg_df is None or session_agg_df.empty:
        return "no_session"
    n = len(session_agg_df)
    td_col = "trade_date" if "trade_date" in session_agg_df.columns else "date"
    if td_col in session_agg_df.columns:
        td_last = str(session_agg_df[td_col].iloc[-1])[:10]
    else:
        td_last = "?"
    return f"s{n}_{td_last}"


def _fast_hint_levels(key_levels: dict | None) -> str:
    """exp-20: Levels hint items (no sort in fast levels hint)."""
    if not key_levels:
        return "no_levels"
    items = [(k, str(v)[:10]) for k, v in sorted(key_levels.items())[:6]]
...
def _stage_cache_get(stage_id: int, fingerprint: str) -> pd.DataFrame | None:
    """Retrieve from stage cache if present."""
    if stage_id not in _STAGE_CACHE:
        return None
    for fp, result in _STAGE_CACHE[stage_id]:
        if fp == fingerprint:
            return result.copy()
    return None


def _stage_cache_put(stage_id: int, fingerprint: str, result: pd.DataFrame) -> None:
    """Store in stage cache, evict oldest if size > 100."""
    if stage_id not in _STAGE_CACHE:
        _STAGE_CACHE[stage_id] = []
    # Evict oldest if over 100 entries
    if len(_STAGE_CACHE[stage_id]) >= 100:
        _STAGE_CACHE[stage_id].pop(0)
    _STAGE_CACHE[stage_id].append((fingerprint, result.copy()))
...
def compute_candle_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    exp-4: Cached by candle fingerprint.
    """
    fp = _fingerprint_df(df)
    cached = _stage_cache_get(4, fp)
    if cached is not None:
        return cached
...
def compute_level_features(df: pd.DataFrame, levels: dict | None = None) -> pd.DataFrame:
    ...
    exp-6:  Cross-session merge output cached by input fingerprints.
    exp-20: Levels hint items (no sort in fast levels hint).
    """
    # exp-6: Cache by df fingerprint + levels hint
    fp = _fingerprint_df(df)
    lvl_fp = _fast_hint_levels(levels)
    cache_key = f"{fp}_{lvl_fp}"
    cached = _stage_cache_get(6, cache_key)
    if cached is not None:
        return cached
    ...
    _stage_cache_put(6, cache_key, df)
    return df
...
def compute_historical_features(
    ...
    """
    Compute features from historical trade log.
    exp-3:  Historical rolling stats cached by trade-log fingerprint.
    exp-11: Skip redundant to_datetime in fast trade/session hints.
    """
    df = df.copy()
    ...
    fp_trade = _fingerprint_df(trade_log)
    if fp_trade in _HIST_STATS_CACHE:
        trade_log = _HIST_STATS_CACHE[fp_trade]
    else:
        # exp-1: Vectorized rolling stats (no .apply for profit_factor)
        trade_log = trade_log.dropna(subset=["entry_time"]).copy()
        entry_ts = trade_log["entry_time"]
        if not pd.api.types.is_datetime64_any_dtype(entry_ts):
            trade_log["entry_time"] = pd.to_datetime(entry_ts)
        ...
        _HIST_STATS_CACHE[fp_trade] = trade_log

    # exp-11: Align timestamps — merge_asof requires matching dtypes.
    # If candles are tz-aware but trades are naive, localize trades to candle tz.
    ...
    tl_dtype = trade_log["entry_time"].dtype
    try:
        tl_tz = tl_dtype.tz
    except (AttributeError, TypeError):
        tl_tz = None
...
def compute_cross_session_features(df: pd.DataFrame, session_agg: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        sess_tz = session_agg["trade_date"].dtype.tz
        if sess_tz is not None:
            session_agg["trade_date"] = session_agg["trade_date"].dt.tz_localize(None)
    except (AttributeError, TypeError):
        pass  # already naive
    df = df.merge(
...
def engineer_features(
    ...
    Performance: exp-9 full-call memoization, exp-10-20 fast-hint path.
    """
    if candles_df.empty:
    ...
    fast_hint = (
        _fast_hint_candles(candles_df),
        _fast_hint_trades(trade_log_df),
    ...
    # Build full cache key from all inputs.
    full_key = "|".join(fast_hint)
    if full_key in _FULL_CACHE:
        return _FULL_CACHE[full_key].copy()
