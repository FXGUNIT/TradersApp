...
HPA_FILE="${REPO_ROOT}/k8s/hpa-ml-engine.yaml"
...
HPA_BEFORE=$(kubectl get hpa ml-engine-hpa -n "$NAMESPACE" -o jsonpath='{.spec.minReplicas}/{.spec.maxReplicas}/{.status.currentReplicas}/{.status.desiredReplicas}' 2>/dev/null || echo "N/A")
...
CURRENT_MIN=$(kubectl get hpa ml-engine-hpa -n "$NAMESPACE" -o jsonpath='{.spec.minReplicas}' 2>/dev/null || echo "")
if [[ -z "$CURRENT_MIN" ]]; then
  warn "HPA ml-engine-hpa not found — applying manifest"
...
kubectl apply -f "$HPA_FILE" --namespace="$NAMESPACE"
log "HPA applied."
kubectl get hpa ml-engine-hpa -n "$NAMESPACE" -o wide 2>&1 | tee "${OUTPUT_DIR}/hpa-before.txt"
...
log "    kubectl get hpa ml-engine-hpa -n $NAMESPACE -w"
...
K6SCRIPT
    ...
    ab)
      ...
      ;;
    curl)
      # Fallback: burst of parallel curls — enough to push CPU utilisation
      for _ in $(seq 1 "$LOAD_CONCURRENCY"); do
        curl -sf -X POST "${ML_ENGINE_URL}/predict" \
          -H "Content-Type: application/json" \
          -d '{"symbol":"MNQ","candles":[]}' &>/dev/null & true
      done
      sleep "$LOAD_DURATION"
      wait
      ;;
...
kubectl get hpa ml-engine-hpa -n "$NAMESPACE" -w &
HPA_WATCH_PID=$!
...
kubectl get hpa ml-engine-hpa -n "$NAMESPACE" -o wide 2>&1 | tee "${OUTPUT_DIR}/hpa-after.txt"
...
FINAL_DESIRED=$(kubectl get hpa ml-engine-hpa -n "$NAMESPACE" -o jsonpath='{.status.desiredReplicas}' 2>/dev/null || echo "0")
